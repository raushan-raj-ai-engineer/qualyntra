...
export interface AlertStateStore { lastTriggered(key:string):Promise<number|undefined>; markTriggered(key:string,at:number):Promise<void>; append(record:AlertRecord):Promise<void>; list(scope?:TenantScope,limit?:number):Promise<AlertRecord[]>; }
export class InMemoryAlertStateStore implements AlertStateStore {
  ...
  async lastTriggered(key:string){return this.last.get(key);}async markTriggered(key:string,at:number){this.last.set(key,at);}async append(record:AlertRecord){this.records.push(structuredClone(record));}
