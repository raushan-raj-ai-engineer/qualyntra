...
export async function verifyControlPlaneRepositoryConformance(repository:ControlPlaneRepository):Promise<RepositoryConformanceResult>{
  ...
  await repository.createRun(run('conformance-r1',p1,'same'));await repository.createRun(run('conformance-r2',p2,'same'));const atomic=await repository.createRunIdempotently(run('conformance-r3',p1,'same'));if(atomic.created||atomic.record.id!=='conformance-r1')throw new Error('Repository failed atomic idempotency conformance.');checks.push('atomic-idempotency');
