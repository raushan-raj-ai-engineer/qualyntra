...
export function createControlPlaneHandler(deps:ControlPlaneRouterDependencies){
  return async function handle(req:any,res:any):Promise<void>{
    ...
    try{
      ...
      if(req.method==='GET'&&(path==='/capabilities'||path==='/api/v1/capabilities')){await requirePermission('platform.read');json(res,200,{apiVersion:'v1',capabilities:{runs:true,results:true,evaluations:true,releaseDecisions:true,governance:true,audit:true,integrations:true,adapters:true,observability:deps.service.observabilityConfigured(),notifications:deps.service.notificationsConfigured()}});return;}
      ...
      if(req.method==='GET'&&path==='/api/v1/notifications'){await requirePermission('platform.read');json(res,200,{items:deps.service.notificationDescriptors()});return;}
      if(req.method==='GET'&&path==='/api/v1/observability/summary'){await requirePermission('platform.read');json(res,200,await deps.service.observabilitySummary(scope));return;}
      if(req.method==='GET'&&path==='/api/v1/alerts'){await requirePermission('platform.read');const page=pagination(url,deps.options.maxPageSize);const records=await deps.service.alertRecords(scope,page.limit);json(res,200,{items:records,offset:0,limit:page.limit,total:records.length});return;}
      if(req.method==='POST'&&parts[0]==='api'&&parts[1]==='v1'&&parts[2]==='notifications'&&parts[4]==='actions'){await requirePermission('notifications.manage');const body=safeObject(await readJson(req,deps.options.maxBodyBytes));const adapterId=parts[3]!;if(typeof body.title!=='string'||typeof body.message!=='string'||!['info','warning','critical'].includes(body.severity))throw new ApiError(400,'invalid_notification','title, message, and severity are required.');json(res,200,await deps.service.sendNotification(adapterId,{id:createId('notification'),title:body.title,message:body.message,severity:body.severity,scope,correlationId,dedupeKey:header(req,'idempotency-key'),attributes:body.attributes,links:body.links}));return;}
      if(req.method==='POST'&&parts[0]==='api'&&parts[1]==='v1'&&parts[2]==='integrations'&&parts[4]==='actions'){await requirePermission('integrations.manage');const body=safeObject(await readJson(req,deps.options.maxBodyBytes));const adapterId=parts[3]!;const response=await deps.service.executeIntegration(adapterId,{operation:body.operation,scope,correlationId,actorId:principal.actor.id,idempotencyKey:header(req,'idempotency-key'),payload:body.payload??{}});json(res,200,response);return;}
      ...
      if(path==='/api/v1/audit'&&req.method==='GET'){await requirePermission('audit.read');const records=await deps.service.auditRecords(scope);const page=pagination(url,deps.options.maxPageSize);json(res,200,{items:records.slice(page.offset,page.offset+page.limit),offset:page.offset,limit:page.limit,total:records.length});return;}
...
export function createBuiltInRoleCatalog():RoleCatalog{
  const all:GovernancePermission[]=['platform.read','execution.run','results.ingest','evaluation.run','governance.read','governance.manage','integrations.manage','notifications.manage','secrets.resolve','release.evaluate','release.approve','audit.read'];
