...
function secureOrigin(value:string,allowInsecure=false):string{const url=new URL(value);const local=['localhost','127.0.0.1','::1'].includes(url.hostname);if(url.protocol!=='https:'&&!(url.protocol==='http:'&&(local||allowInsecure)))throw new Error('QUALYNTRA_AGENT_CONTROL_PLANE_ORIGIN must use HTTPS outside localhost unless explicitly enabled for a trusted internal network.');return url.origin;}
export function loadExecutionAgentConfiguration(env:Record<string,string|undefined>=process.env):ExecutionAgentConfiguration{
  ...
  return{controlPlaneOrigin:secureOrigin(env.QUALYNTRA_AGENT_CONTROL_PLANE_ORIGIN?.trim()||'http://127.0.0.1:4317',bool(env.QUALYNTRA_AGENT_ALLOW_INSECURE_CONTROL_PLANE,false)),scope:{organizationId,workspaceId,projectId,environmentId},dataDir,workspaceRoot:env.QUALYNTRA_AGENT_WORKSPACE_ROOT?.trim()||`${dataDir}/work`,secretRoot:env.QUALYNTRA_AGENT_SECRET_ROOT?.trim()||'/run/secrets',tokenFile,token,healthHost:env.QUALYNTRA_AGENT_HEALTH_HOST?.trim()||'127.0.0.1',healthPort:integer(env.QUALYNTRA_AGENT_HEALTH_PORT,4321,'QUALYNTRA_AGENT_HEALTH_PORT',1,65535),pollMs:integer(env.QUALYNTRA_AGENT_POLL_MS,1_000,'QUALYNTRA_AGENT_POLL_MS',100,60_000),heartbeatMs,leaseMs,workerStaleMs:integer(env.QUALYNTRA_AGENT_WORKER_STALE_MS,60_000,'QUALYNTRA_AGENT_WORKER_STALE_MS',leaseMs,900_000),retryDelayMs:integer(env.QUALYNTRA_AGENT_RETRY_DELAY_MS,1_000,'QUALYNTRA_AGENT_RETRY_DELAY_MS',0,300_000),maxConcurrency:integer(env.QUALYNTRA_AGENT_MAX_CONCURRENCY,1,'QUALYNTRA_AGENT_MAX_CONCURRENCY',1,128),maxWorkspaceBytes:integer(env.QUALYNTRA_AGENT_MAX_WORKSPACE_BYTES,2_147_483_648,'QUALYNTRA_AGENT_MAX_WORKSPACE_BYTES',1_048_576,107_374_182_400),shutdownGraceMs:integer(env.QUALYNTRA_AGENT_SHUTDOWN_GRACE_MS,30_000,'QUALYNTRA_AGENT_SHUTDOWN_GRACE_MS',1_000,900_000),cleanupWorkspaces:bool(env.QUALYNTRA_AGENT_CLEANUP_WORKSPACES,true),labels:list(env.QUALYNTRA_AGENT_LABELS),languages:list(env.QUALYNTRA_AGENT_LANGUAGES),engines:list(env.QUALYNTRA_AGENT_ENGINES),runnerIds:list(env.QUALYNTRA_AGENT_RUNNERS),allowEnvSecrets:bool(env.QUALYNTRA_AGENT_ALLOW_ENV_SECRETS,false)};
}
