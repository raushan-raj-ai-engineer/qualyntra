...
export class S3ArtifactStorageAdapter implements ArtifactStorageAdapter{
  ...
  async health():Promise<AdapterHealth>{try{this.network();const healthy=this.client.health?await this.client.health(this.config.bucket):true;return{status:healthy?'healthy':'degraded',checkedAt:new Date().toISOString()};}catch(error){return{status:'unavailable',checkedAt:new Date().toISOString(),message:error instanceof Error?error.message:'S3 artifact storage unavailable'};}}
...
export { AwsSdkS3ArtifactClient } from './aws-sdk-client';
