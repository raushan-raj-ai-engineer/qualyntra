...
for(const file of ['deploy/docker/control-plane.Dockerfile','deploy/docker/dashboard.Dockerfile','deploy/compose/docker-compose.yml','deploy/helm/qualyntra/Chart.yaml','deploy/helm/qualyntra/values.yaml','deploy/helm/qualyntra/values.schema.json'])if(!fs.existsSync(file))failures.push(`${file}: missing`);
for(const file of ['deploy/docker/control-plane.Dockerfile','deploy/docker/dashboard.Dockerfile','apps/agent/Dockerfile'])requireText(file,['USER qualyntra','HEALTHCHECK']);requireText('deploy/docker/control-plane.Dockerfile',['migrations/postgres','QUALYNTRA_POSTGRES_MIGRATIONS_DIR']);
requireText('deploy/compose/docker-compose.yml',['read_only: true','cap_drop: ["ALL"]','no-new-privileges:true','condition: service_healthy','QUALYNTRA_RUNTIME_BACKEND: memory']);
...
requireText('deploy/helm/qualyntra/values.yaml',['runtimeBackend:','mode: memory','connectionSecret:']);requireText('deploy/helm/qualyntra/templates/control-plane.yaml',['QUALYNTRA_RUNTIME_BACKEND','QUALYNTRA_POSTGRES_CONNECTION_STRING_FILE','postgres-secret']);
