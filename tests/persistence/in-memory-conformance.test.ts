...
test('in-memory repository satisfies persistence conformance',async()=>{const result=await verifyControlPlaneRepositoryConformance(new InMemoryControlPlaneRepository());assert.deepEqual(result.checks,['atomic-idempotency','idempotency-scope','tenant-visibility','optimistic-version']);});
