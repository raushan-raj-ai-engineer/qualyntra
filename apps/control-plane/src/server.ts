...
const audit=new InMemoryAuditLog();const roles=createBuiltInRoleCatalog();const governance=new GovernanceService(new RbacAuthorizer(roles),audit);const registry=new AdapterRegistry();const integrations=new IntegrationService(registry,audit);const notifications=new NotificationService(registry,audit);const telemetry=new InMemoryTelemetryStore();const alertState=new InMemoryAlertStateStore();const observability=new ObservabilityService(telemetry,telemetry,new AlertEngine([],alertState,notifications),alertState);const repository=new InMemoryControlPlaneRepository();const service=new ControlPlaneService(repository,governance,registry,integrations,audit,observability,notifications);const rateLimiter=new FixedWindowRateLimiter(config.controlPlane.rateLimitPerMinute);
