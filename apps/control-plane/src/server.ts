...
const audit=new InMemoryAuditLog();const roles=createBuiltInRoleCatalog();const governance=new GovernanceService(new RbacAuthorizer(roles),audit);const registry=new AdapterRegistry();const integrations=new IntegrationService(registry);const repository=new InMemoryControlPlaneRepository();const service=new ControlPlaneService(repository,governance,registry,integrations,audit);const rateLimiter=new FixedWindowRateLimiter(config.controlPlane.rateLimitPerMinute);
